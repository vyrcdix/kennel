...
### 6. Set-a-bearing creation flow · `SetBearingSheet` (built live)
The deliberate, high-friction inverse of capture. Two beats — **Compose**
(statement-as-direction + verb starters, optional finish-line toggle [default
off], optional current-picker to seed the roll-up) → **Sit with it** (a confirm
beat naming that bearings are load-bearing + rarely changed; Save as draft vs
Set this bearing), plus a "N of ~6 · few by design" counter. Two entry points
(here + crystallize-an-item-as-orientation, pre-filled); editing / Reword reuse
the same sheet. See **`ANSWERS to engineering handback.md`** §A5.
...
**Most of these are now resolved** in `ANSWERS to engineering handback.md`
(both-skin parity → both, vocabulary-localized; schema → confirmed as proposed,
one combined migration; scope → five core surfaces this round; A5 flow → built).
Still genuinely open, needing a yes/no:

1. **Let-go behavior (B1):** when a bearing is let go, do its promises revert to
   `role = practice` (our lean) or release?
2. **Persona insurance (B4):** key bearings by persona/workspace from day one
   (cheap) even though one set renders now?
3. **Naming depth:** does the data-model noun (+ the System/Studies docs) adopt
   **current**, or does it stay a UI-copy-only term? (copy is swept; identifiers
   aren't.)
...
- `ANSWERS to engineering handback.md` — **point-by-point replies** to the
  engineering review (A1–A5, B1–B4), incl. the Life/Workshop `copy.ts` table.
